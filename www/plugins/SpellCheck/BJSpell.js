...
BJSpell = function () {
    ...
    function BJSpell(dic, callback) {
        if (this instanceof BJSpell) {
            ...
            if (BJSpell[lang]) {
                ...
                self.originalword = {};
                var keys = self.keys = [],
                    mistakeys = self.mistakeys = [],
                    words = self.words,
                    i = 0,
                    commonmistake = self.suggestedword,
                    key;
                for (key in commonmistake)
                    mistakeys[i++] = key;
                mistakeys.indexOf = indexOf;
    ...
    */
    BJSpell.prototype.check = function (word) {
        ...
        return typeof checked === "boolean" ? checked : this.parse(word, orgiword);
    };
    ...
    * @return Boolean false if the word does not exist
    */
    BJSpell.prototype.parse = function (word, orgiword) {
        if (/^[0-9]+$/.test(word))
        ...
        this.originalword[orgiword] = result;
    ...
    */
    BJSpell.prototype.suggest = function (word, many) {
        if (typeof this.words[word] === "string") {
            ...
            var keys = this.keys,
                length = keys.length,
                mykeys = this.mistakeys,
                mykeyslength = mykeys.length,
                i = Math.abs(many) || 1,
                ceil, indexOf, words;
            word = word.toLowerCase();
            
            var found = lang.suggestedword[word];
